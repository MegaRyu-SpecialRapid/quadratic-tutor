module.exports = async function (context, req) {
  try {
    ...
    if (a === 0 && b === 0) return ok(context, '二次関数の形 y=ax^2+bx+c を検出できませんでした。例: y=2x^2-3x+1');
    if (a === 0) return ok(context, 'a=0 のため一次関数です。二次関数 y=ax^2+bx+c の形で入力してください。');
    ...
    return ok(context, answer);
  } catch (err) {
    context.log('solve error:', err);
    context.res = { status: 500, headers: {'Content-Type':'application/json'}, body: { error: String(err) } };
  }
};

function ok(context, answer){
  context.res = {
    status: 200,
...
function fmtSign(n, withZero=false){ const s = (n>=0? '+' : ''); if (!withZero && n===0) return ''; return s + fmt(n); }
function help(){ return '二次関数 y=ax^2+bx+c を含む質問に答えます。例: 「y=2x^2-3x+1 の頂点と解」'; }
``
